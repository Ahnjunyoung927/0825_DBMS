...
-- 실습문제
-- 1. JOB 테이블에서 JOB_CODE, JOB_NAME컬럼을 조회하세요
SELECT 
       JOB_CODE
     , JOB_NAME
  FROM 
...
-- 5. EMPLOYEE 테이블에서 HIRE_DATE, EMP_NAME, SALARY 칼럼을 조회
SELECT 
       HIRE_DATE
     , EMP_NAME
     , SALARY
...
-- EMPLOYEE 테이블로부터 사원명, 월급, 보너스를 포함한 연봉(월급 * 보너스 + 월급) * 12
SELECT
       EMP_NAME
     , SALARY
     , BONUS
...
-- 4. EMPLOYEE 테이블에서 연봉이 5000이상인 사원들의 이름 연봉 조회
...
/*
 * < LIKE '특정패턴' >
 * 비교하려는 칼럼의 값이 내가 지정한 특정 패턴에 만족할 경우 조회
 * 
 * [ 표현법 ]
 * 비교대상컬럼 LIKE '특정패턴'
 * 
 ...
 * 
 * '%' : 0글자 이상
 *       비교대상컬럼 LIKE 'A%' => 컬럼값 중 'A'로 시작하는 것만 조회
 *       비교대상컬럼 LIKE '%A' => 컬럼값 중 'A'로 끝나는 것만 조회
 *       비교대상컬럼 LIKE '%A%' => 컬럼값중 'A'가 포함되는 것을 전부 조회
 * '_' : 1글자
 * 비교대상 컬럼 LIKE '_A' => 컬럼값에 'A'앞에 무조건 1글자가 있어야만 패턴에 만족
 * 비교대상 컬럼 LIKE '__A' => 컬럼값에 'A'앞에 무조건 2글자가 있어야만 패턴에 만족
 */
...
-- 이름의 두번째 글자가 '승'인 사원들의 사원명, 핸드폰 번호
SELECT 
       EMP_NAME
     , PHONE
  FROM
...
SELECT 
 ...
 WHERE 
       PHONE NOT LIKE '010%';
-- 4. DEPARTMENT 테이블로부터 해외영업과 관련된 부서들 조회
...
SELECT  
  ...
  FROM 
       EMPLOYEE
 WHERE
    -- BONUS IS NULL; -- NULL을 비교할 땐 = 동등비교 연산자가 아닌 IS NULL을 써야함
       BONUS IS NOT NULL;
 ...
 * 3. 연결연산자
 * 4. 비교연산자
 * 5. IS NULL, LIKE, IN (우선순위 같음, 앞에 있는 것부터)
 * 6. BETWEEN AND
 * 7. NOT
 * 8. AND
 * 9. OR
...
-- 첫 번쨰로 제시한 정렬기준의 컬럼값이 동일할 경우 다음 정렬기준을 명시할 수 있음
