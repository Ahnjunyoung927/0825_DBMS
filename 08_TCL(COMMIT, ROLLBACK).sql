 ...
 * - 트랜잭션으로 대상이 되는 SQL : INSERT, UPDATE, DELETE
 * 
 ...
 *                      시스템에 문제가 발생하더라도 영구적으로 반영이 되어야 한다.
 */
...
UPDATE EMP_COPY SET EMP_NAME = '홍길동' WHERE EMP_NAME = '고길동'; -- 트랜잭션에 추가

SELECT * FROM EMP_COPY;

COMMIT; -- 트랜잭션에 있는 내용을 확정 (저장매체에 저장)
ROLLBACK;
 ...
 * 트랜잭션에 있는 모든 작업사항을 무조건 COMMIT해서 실제 DB에 반영 한후 DDL을 수행
